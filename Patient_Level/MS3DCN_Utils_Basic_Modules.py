...
def multi_scale_get_model_DCN(width=128, height=128, depth=128, batch_size=2, num_class=3, factor=2, num_gpu=2):
    inputs_A = Input(shape=(width, height, depth, 1))
    ...
    inputs_A_resized = Lambda(lambda x: keras.backend.resize_volumes(x,
                                                                     width_factor=1,
                                                                     height_factor=2,
                                                                     depth_factor=2,
                                                                     data_format='channels_last'))(inputs_A)
    # (B, 128, 128, 128, 1) --> (B, 256, 256, 128, 1)
    ...
    # inputs_A: (B, 128, 128, 128, 1)
    x1_3d_gap = conv3d_module_from_3D_input(inputs_A, factor=factor,
                                            attention_type='eca_block', stage=4, conv_block=2)

    x1_2d = conv2D_module_from_3D_input(inputs_A_to_2d, factor=factor,
                                        attention_type='eca_block', stage=4, conv_block=2)
    x1_2d_flatten = Lambda(lambda x: keras.backend.reshape(x,
                                                           shape=(-1, x.shape[1] * x.shape[2] * x.shape[3])))(x1_2d)
    # Flatten()(x1_2d)    # (B, H_o, W_o, C) ---> (B, H_o * W_o * C)

    x1_p3d = convp3d_module_from_3D_input(inputs_A_to_p3d, factor=factor,
                                          attention_type='eca_block', stage=4, conv_block=2)
    # x1_p3d_gap = Lambda(lambda x: keras.backend.reshape(x, shape=(-1, x.shape[1]*x.shape[2]*x.shape[3])))(x1_p3d)
    x1_p3d_gap = GlobalAveragePooling2D()(x1_p3d)

    # inputs_A_resize: (B, 256, 256, 128, 1)
    x2_3d_gap = conv3d_module_from_3D_input(inputs_A_resized, factor=factor,
                                            attention_type='eca_block', stage=4, conv_block=2)

    x2_2d = conv2D_module_from_3D_input(inputs_A_resized_2d, factor=factor,
                                        attention_type='eca_block', stage=4, conv_block=2)
    x2_2d_gap = Lambda(lambda x: keras.backend.reshape(x, shape=(-1, x.shape[1]*x.shape[2]*x.shape[3])))(x2_2d)


    x2_p3d = convp3d_module_from_3D_input(inputs_A_resized_p3d, factor=factor,
                                          attention_type='eca_block', stage=4, conv_block=2)
    # x2_p3d_gap = Lambda(lambda x: keras.backend.reshape(x, shape=(-1, x.shape[1]*x.shape[2]*x.shape[3])))(x2_p3d)
    x2_p3d_gap = GlobalAveragePooling2D()(x2_p3d)

    batch_size_gpu = batch_size // num_gpu
    print(batch_size_gpu)
    if batch_size_gpu > 1:
        x1_2d_gap_reshape = Lambda(lambda x: keras.backend.reshape(x, (-1, 128, x.shape[1])))(x1_2d_flatten)
        x1_2d_gap_gap = GlobalAveragePooling1D()(x1_2d_gap_reshape)
        print(x1_2d.shape, x1_2d_flatten.shape, x1_2d_gap_reshape.shape, x1_2d_gap_gap.shape)

        x1_p3d_gap_reshape = Lambda(lambda x: keras.backend.reshape(x, (-1, 63, x.shape[1])))(x1_p3d_gap)
        x1_p3d_gap_gap = GlobalAveragePooling1D()(x1_p3d_gap_reshape)

        x2_2d_gap_reshape = Lambda(lambda x: keras.backend.reshape(x, (-1, 128, x.shape[1])))(x2_2d_gap)
        x2_2d_gap_gap = GlobalAveragePooling1D()(x2_2d_gap_reshape)

        x3_p3d_gap_reshape = Lambda(lambda x: keras.backend.reshape(x, (-1, 63, x.shape[1])))(x2_p3d_gap)
        x2_p3d_gap_gap = GlobalAveragePooling1D()(x3_p3d_gap_reshape)
    else:
        print(x1_2d_flatten.shape, "Line-584")
        x1_2d_gap_reshape = Lambda(lambda x: keras.backend.reshape(x, shape=(-1, 128, x.shape[1])))(x1_2d_flatten)
        x1_2d_gap_gap = GlobalAveragePooling1D()(x1_2d_gap_reshape)

        x1_p3d_gap_reshape = Lambda(lambda x: keras.backend.reshape(x, shape=(-1, 63, x.shape[1])))(x1_p3d_gap)
        x1_p3d_gap_gap = GlobalAveragePooling1D()(x1_p3d_gap_reshape)

        x2_2d_gap_reshape = Lambda(lambda x: keras.backend.reshape(x, shape=(-1, 128, x.shape[1])))(x2_2d_gap)
        x2_2d_gap_gap = GlobalAveragePooling1D()(x2_2d_gap_reshape)

        x2_p3d_gap_reshape = Lambda(lambda x: keras.backend.reshape(x, shape=(-1, 63, x.shape[1])))(x2_p3d_gap)
        x2_p3d_gap_gap = GlobalAveragePooling1D()(x2_p3d_gap_reshape)

        
    x_c = concatenate([# x1_3d_gap,
                       # x1_2d_gap_gap,
                       x1_p3d_gap_gap,
                       # x2_3d_gap,
                       # x2_2d_gap_gap,
    ...
    print(x_c.shape, "Line-618")

    x_d = Dense(units=1024, activation="relu")(x_c)
    x_d = Dense(units=1024, activation="relu")(x_d)
    if num_class == 2:
        outputs_class = Dense(units=1, activation="sigmoid")(x_d)
    else:
        outputs_class = Dense(units=num_class, activation="softmax")(x_d)

    # Define the model.
    model = Model(inputs=inputs_A, outputs=outputs_class, name="3dcnn")
