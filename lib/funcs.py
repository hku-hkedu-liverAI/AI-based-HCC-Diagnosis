...
def predict_window_mulgpu(model, batch, imgs_test, img_deps, img_rows, img_cols, multiloss):
    windows_deps, windows_rows, windows_cols = (img_deps/3)*2, (img_rows/3*2),(img_cols/3*2)

    current_test = imgs_test
    x, y, z = current_test.shape[0], current_test.shape[1], current_test.shape[2]
    ...
    box_test = np.zeros((batch, img_deps, img_rows, img_cols, 1), dtype="float32")
    for deps in range(0, y-img_rows+windows_rows, windows_rows):
        for cols in range(0, z-img_cols+windows_cols, windows_cols):
            if deps>x-img_deps:
                deps = x-img_deps
